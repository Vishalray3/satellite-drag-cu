function [relsv] = svrelativity(ephem_all,t_input, prn)
%==========================================================================
%==========================================================================
...
for tt = 1:sz % loop through all input times
    ...
    asqr   = sat_ephem(jj,5);          % square root of semimajor axis, sqrt(a) (meters)^0.5
    ecc = sat_ephem(jj,4);             % eccentricity
    n0  = sqrt(muE)/asqr^3;            % nominal mean motion (rad/s)
    ...
    % Satellite Relativistic Correction
    relsv(tt,1) =  c*(-2/c^2*sqrt(muE)*asqr*ecc*sinE); % in m
