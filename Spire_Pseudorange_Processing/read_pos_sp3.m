...
if fid == -1
    ...
    while (isempty(file_end) == 1) && ~feof(fid)
        ...
        if (isempty(pos_line) == 0)
            [sat_ID, remain] = strtok(current_line); sat_ID = str2num(sat_ID(2:end));
            ...
            data_ = [gpswk gpssec sat_ID str2num(x) str2num(y) str2num(z) str2num(b)]';
        end
        ...
        if (isempty(vel_line) == 0)
            ...
            data(:,j) = [data_;str2num(xd);str2num(yd);str2num(zd)];
