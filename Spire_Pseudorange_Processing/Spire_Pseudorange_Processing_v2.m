function [data_] = Spire_Pseudorange_Processing_v2(year,month,dayy,sat_ID,pos_iter,vel_iter,flag_w, dir_rnx, dir_sp3_q, dir_GPSn)
%==================================================
...
global Omega_E c f1 f2 eop nut80 eqeterms
...
recef = []; vecef = []; dpost_p=[];  dpre_p=[]; dpost_v = []; freq_diff = [];
...
for tm = 1 : num_mon % months loop
    ...
    for td = 1 : numel(dd_tm) % days loop
        ...
        %%%%%%% ================== Read GPS navigation file ====================
        S_GPSn= dir(dir_GPSn); N_GPSn= {S_GPSn.name}; % access to GPS nav files
        doy = day(datetime(date),'dayofyear'); % day of year for GPS
        [~, ind_GPSn] = ismember(['brdc' num2str(doy) '0.18n'], N_GPSn);% find index of GPS file
        filename_GPSn = [dir_GPSn N_GPSn{ind_GPSn}]; % GPS nav file name
        % Read GPS broadcast ephemeris :
        [gps_ephem,~]=read_clean_GPSbroadcast(filename_GPSn,true);
...
%              data(7,:) = data(7,:) +20/c*1e6; 
            br = interp1(tsp3 + data(7,:)*1e-6,data(7,:),trnx,'spline'); % zeros(1, numel(trnx)); %
            trnx = trnx - br*1e-6;
            tJD = tJD - br*1e-6/86400;
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
            xr = interp1(tsp3,data(4,:),trnx,'spline');yr = interp1(tsp3,data(5,:),trnx,'spline');
            zr = interp1(tsp3,data(6,:),trnx,'spline');br = interp1(tsp3,data(7,:),trnx,'spline');
            xdr = interp1(tsp3,data(8,:),trnx,'spline');ydr = interp1(tsp3,data(9,:),trnx,'spline');
            zdr = interp1(tsp3,data(10,:),trnx,'spline');
            ...
            DPr_pre = nan(32,num_epochs); DPr_post = DPr_pre; range_freq = DPr_pre;
            ...
            for r=1:num_epochs
                ...
                range_err = PIF - C1; %C1-C2;
%                 PIF = C2;
                ind_PIF = find(~isnan(PIF)); PIF = PIF(ind_PIF); range_err = range_err(ind_PIF);
...
% [~,~, T_ecefr2eci] = ecef2eci  ( [0;0;0], [0;0;0],ttt,jdut1,lod,xp,yp,eqeterms,dpsi, deps,nut80 );
                ...
                for i=1:numel(prn)
                    ...
                    [~, pos,vel] = broadcast_eph2pos_vel(gps_ephem,[wkn tow],PRN);
                    ...
                    while and(error>1e-9,counter < 10)
                        ...
                        Tt(i)=tow-R/c;
%                         [bsv(i),a1(i),GD(i)] = svclock(gps_ephem, [wkn Tt(i)], PRN);
                        
%                         Tt(i) = Tt(i) - bsv(i)/c;
                        [~, pos,vel] = broadcast_eph2pos_vel(gps_ephem,[wkn Tt(i)],PRN);
                        
                        satECEF(:,i)=pos';
                        vsatECEF(:,i) = vel';

%                         Jd_new = JD(r)*86400 - R/c;
%                         jdgps_sec = Jd_new;
% jdutc_sec = jdgps_sec - eop.dat + 19;
% % Julian Centuries past 1-Jan-2000 12:00 Terrestrial Time
% ttt = (jdutc_sec + eop.dat + 32.184 - 86400*2451545.0)/86400/36525; % sec
...
% dpsi = (pi/180/3600/1000)*interp1(86400*eop.fmjd,eop.dpsi,jdutc_sec-86400*2400000.5,'linear'); % now in radians (from milli-arc-sec)
% deps = (pi/180/3600/1000)*interp1(86400*eop.fmjd,eop.deps,jdutc_sec-86400*2400000.5,'linear'); % now in radians (from milli-arc-sec)
% [~,~, T_eceft2eci] = ecef2eci  ( satECEF(:,i), [0;0;0],ttt,jdut1,lod,xp,yp,eqeterms,dpsi, deps,nut80 );
% 
% Rot3 =  (T_eceft2eci*T_ecefr2eci')';
                        ...
                        satECEF_new=Rot3*satECEF(:,i);
                        range1=norm(satECEF_new-userECEF_);
                        error=abs(range1-R)/R;
                        R=range1;
                        counter=counter+1;
                    end
                    satECEF(:,i) = satECEF_new;
                    % Transmitter Clock corrections
                    
                    [bsv(i),a1(i),GD(i)] = svclock(gps_ephem, [wkn Tt(i)], PRN);
                    
                    % Relativistic corrections
                    relsv(i) = svrelativity(gps_ephem, [wkn Tt(i)], PRN);
                ...
                while flag == 0 && iter <pos_iter
                    ...
                    ind_El = find(El>-5);
                    if numel(ind_El) >=5
                        PIF = PIF(ind_El); RANGE = RANGE(ind_El); range_err = range_err(ind_El);
            ...
            qq = [qq,qs]; dpost_p = [dpost_p,DPr_post]; dpre_p = [dpre_p,DPr_pre]; dpost_v = [dpost_v,DPrr_post]; freq_diff = [freq_diff, range_freq];
