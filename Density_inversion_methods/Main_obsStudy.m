...
load('Cd_sphere_dria_150F10','Af_total_mat','Af_mean','Af_std','Bf_mean','Bf_std','Kl_mat')
% load('Cd_cube_dria_150F10','Af_std','Bf_std','Cf_std','Df_std','Kl_mat','Af_Kl','Bf_Kl','Af_total_mat','Bf_total_mat','Cf_total_mat','Df_total_mat')
% load('Cd_gpm_dria_150F10','Af_std','Bf_std','Cf_std','Df_std','Kl_mat','Af_Kl','Bf_Kl','Af_total_mat','Bf_total_mat','Cf_total_mat','Df_total_mat')
truth_model =  'truth_msis00'; %'Truth_sphere_dria';%'Truth_gpm_dria_iner_bff';  %;
%% Flags
case_run = 'estimation';                % 'truth', 'estimation', 'consider'
...
flag_drag = 'Orb';                 % body model: Bod, orbit model: Orb; Body orbit model: Bod_orb
...
epoch_utc     = 'Mar 23, 2007 00:00:00 UTC';             %% current epoch
...
doy = 82;                                     %% day of year (nrlmsise-00)
year = 2007;                                %% year (nrlmsise00)

eps  = 1000;                                   %% altitude diff. in m

%% Body properties for SRP
% N_plates = 10;                          % 6 body plates and 4 sp plates
% area_vec(:,1) = [1;0;0];
% area_vec(:,2) = [-1;0;0];
% area_vec(:,3) = [0;1;0];
% area_vec(:,4) = [0;-1;0];
...
% % canted solar panel area vector will be calculated at each instant
% spec_ref = [0.24, 0.25, 0.28, 0.26, 0.31, 0.27, 0.19, 0.21, 0.19, 0.21];        % specular reflectivites
% diff_ref = [0.10, 0.12, 0.11, 0.10, 0.09, 0.10, 0.12, 0.00, 0.12,0.00];                  % diffuse reflectivities
% Area_plates = [1.5*2+1, 1.5*2+0.75, 2*2.5 + 1.5, 2*2.5 + 1.0, 2.5*1.5, 2.5*1.5+0.1, 13.5, 13.5, 13.5, 13.5];      % solar panel areas = 13.5
% flag_axis = 'y';             % for flag y, i.e. rotation axis y, theta is defined as the angle between vector projection in x-z plane and z-axis

%% Station coordinates in ecef frame and noise values, actual GPM values
...
switch case_run
    ...
    case {'estimation','consider'}   
        load(truth_model,'X_true','meas','y_meas','R_aug','Af_true','Bf_true','Cf_true','Df_true','Kl_mat','K_ind','Cd_true')
        if strcmp(flag_drag, 'Bod_orb')
            Af_total = Af_total_mat(:,:,100);
            Bf_total = Bf_total_mat(:,:,100);
            Cf_total = Cf_total_mat(:,:,100);
            Df_total = Df_total_mat(:,:,100);            
        end
        Af_total = Af_true; Bf_total = Bf_true; Cf_total = Cf_true; Df_total = Df_true;
        Cf_std = zeros(Order_b+1,Order_o+1);
        Df_std = zeros(Order_b+1,Order_o+1);
end
...
% Cd_std = 1e4; %10*Af0_true;
% Xf_std = 1e4*ones(1,numel(Xf_std)); % 10*Xf_true; 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
