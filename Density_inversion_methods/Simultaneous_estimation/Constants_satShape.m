...
% load EIGEN-2
addpath('/Users/vishalray/GoogleDrive/Vishal/PhD/Simulations/Density inversion methods/Simultaneous estimation/Simulation')
load GravCoeff_EGM2008_zeroTide
load fes2010_oceantide
load('case10_truth', 'reci', 'veci', 'theta','rot_SBF2ECI','phi','rho')
% load('Cd_simSpire500_orb','Af_total_mat','Af_std','Bf_total_mat','Bf_std','Cf_total_mat','Cf_std','Df_total_mat','Df_std')
% rot_SBF2ECI = eye(3);
% theta = 0;
% phi = 0;
Cd_discrete = 0;
...
if strcmp(case_run, 'Truth')
    load('Cd_simSpire_2003_jb08_grace','Af_total_mat','Bf_total_mat','Cf_total_mat','Df_total_mat')
    Af_std = abs(Af_total_mat(:,:,1));
    Bf_std = abs(Bf_total_mat(:,:,1));
    Cf_std = abs(Cf_total_mat(:,:,1));
    Df_std = abs(Df_total_mat(:,:,1));
    K_ind = 3;                           % true drag-coeff selection
    estimated_coeff = [{'Cd'}]; %[{'rho_DMC'},{'Cd'}];        % 'Cr','Cd','rho_DMC', 'cd_DMC', 'CdTrue', 'FourierDMC', 'CdDiscrete', 'CdDiscrete_est'
    flag_rho = 'MSIS00';                  % 'MSIS00', 'JB08', 'Exp'
    Kl = 0;    %1.44e6                      % Mehta and walker .  % . Sesam 5e6/133.322;
    del_r = [0;0;0];     % initial standard deviations
    del_v = [0;0;0];
else
    load('Cd_observable_trial','Af_total_mat','Bf_total_mat','Cf_total_mat','Df_total_mat')
    estimated_coeff = [{'Cd'}]; %[{'rho_DMC'},{'Cd'}]; 
    K_ind = 1;                           % true drag-coeff selection
    flag_rho = 'HASDM_dis';                  % 'MSIS00', 'JB08', 'Exp'
...
deg_grav = 2;                                           %% degree of spherical harmonics
ord_grav = 2;                                           %% order of gravity harmonics
...
%% SRP/drag Parameters
if ismember('CdDiscrete_est', estimated_coeff)
    Cd_nom = 1;
else
    Cd_nom = Af_total_mat(1,1,K_ind);%0.46; %0.37; %0.23; % 0.276
end
...
% area_vec(:,1) = [1;0;0];
% area_vec(:,2) = [-1;0;0];
% area_vec(:,3) = sqrt(2)/2*[1;1;0];
% area_vec(:,4) = sqrt(2)/2*[1;-1;0];
% area_vec(:,5) = sqrt(2)/2*[1;0;1];
% area_vec(:,6) = sqrt(2)/2*[1;0;-1];
area_vec(:,1) = [1;0;0];
area_vec(:,2) = [-1;0;0];
area_vec(:,3) = sqrt(2)/2*[1;1;0];
area_vec(:,4) = sqrt(2)/2*[1;-1;0];
area_vec(:,5) = [cosd(60);0;sind(60)];
area_vec(:,6) = [cosd(30);0;-sind(30)];
...
Area_plates = [1, 0.5*(1+2*cosd(45)/sind(60) + 1+2*cosd(45)/sind(30))*(1+1/tand(60)+1/tand(30)), 0.5*(1+1/tand(60)+1/tand(30)), ...
               0.5*(1+1/tand(60)+1/tand(30)), 0.5*sind(45)/sind(60)*(1+2*cosd(45)/sind(60)), 0.5*sind(45)/sind(30)*(1+2*cosd(45)/sind(30))]...
                /area*area_factor;
% Area_plates = [1,4,0.5*1*(2+1),0.5*1*(2+1),0.5*1*(2+1),0.5*1*(2+1)]/area*area_factor;
Ar = 1;

spec_ref = [0.2, 0.05, 0.05, 0.05, 0.05, 0.05];
diff_ref = [0.4, 0.3, 0.3, 0.3, 0.3, 0.3];
N_plates = numel(spec_ref);
flag_axis = 'y';

M_s = [26.981538, 60.0843, 60.0843, 60.0843, 60.0843, 60.0843];             % asymmetry test case
...
sig_meas(1) = 1e-1; %3;  ; found by using the evar function on the residuals of the real data for order 150
sig_meas(2) = 1e-3; % 0.01; %  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% noise mismatch
...
switch flag_rho
    ...
    case 'HASDM'
        load 2003_HASDM_400-475KM                 %%%%%%%%%%%%% check for different altitudes
        ind_day = [];
        for nn = 1:n_days
            doy_hasdm = doy+nn-1;
            ind_day  = [ind_day;find(den_mat(:,2) == doy_hasdm)];
        end
        ind_alt1 = find(den_mat(:,7) == 450);
        ind_alt2 = find(den_mat(:,7) == 475);
        ...
        hasdm_mat = [den_mat(ind_day_alt1,:);den_mat(ind_day_alt2,:)];
                ...
                ind_day  = find(den_mat(:,2) == doy);
                ind_alt1 = find(den_mat(:,7) == 500);
                ind_day_alt1 = intersect(ind_day, ind_alt1);
            ...
            for ii_njd = 1:numel(vec_njd)
                ...
                for ii_long = 1:numel(vec_long)
                    ...
                    for ii_lat = 1:numel(vec_lat)
                        ...
                        den_grid(ii_alt,ii_njd,ii_long,ii_lat) = hasdm_mat(ind_den,11);
                    end
                end
            end
        end
        gridVecs = {vec_alt,vec_njd,vec_long,vec_lat};
        parameters.F_hasdm = griddedInterpolant(gridVecs,log(den_grid));
...
parameters.theta = repmat(theta, N_plates,1);
% parameters.angle_panels = angle_panels;
parameters.phi = repmat(phi, N_plates,1);
