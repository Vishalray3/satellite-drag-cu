...
write_to_api(sprintf("'Running toolkit for Spire satellite ID: %s'", sat_ID),flag_disp)
% pause(1)
stoch_threshold = 10*60;   % 10 minutes
%%
...
X_est = zeros(N_est,N_t);
R_aug = R_aug(vec_meas,vec_meas);
t_meas = zeros(1,N_t);
X_full_uni = zeros(N_st + 2*N_est^2+N_est,numel(time_prop_utc));
X_full =  zeros(N_st + 2*N_est^2+N_est,N_t);
X_ref =  zeros(N_st,N_t);
b_corr = zeros(N_est,N_t);
y_pred = zeros(numel(vec_meas),N_t);
smoother(N_t) = struct();
Xs_est = zeros(N_est,N_t);
ys_res_postfit = zeros(numel(vec_meas),N_t);
sigma_xs = zeros(N_est,N_t);
for iter=1:N_iter
    iteration = iter;
%     write_to_api(sprintf("'Iteration = %i'", iteration),flag_disp)
    write_to_api(sprintf("' Stage %i: Forward filtering'", iteration),flag_disp)
    P_up = P_prior;
        ...
        if isnan(time_prop_utc_ekf(i))
            ...
            if time_prop_utc_ekf(i) == 0
                ...
                if t_meas(k+1) - t_meas(k) <= 10
                    ...
                    X1 = ode4(propagator_dmc_real,ode_step,X_aug(:,1),parameters,Tsamp);        % run offline trajectory
                    %                 [t,X1] = ode45(@(t,x)propagator_dmc(t, x, parameters), ode_step, X_aug(:,1), options);
                    X_aug = X1(end,:)';
                    X_full_uni(:,ind_f) = X1(2,:)';
                    time_new(ind_f) = ode_step(2);
                    ind_f = ind_f +1;
                else
...
%                     X_ref(8:10,k) = 0;
                    ...
                    X_aug = X_aug';
                    X_full_uni(:,ind_f:ind_f+numel(ode_step)-3) = X1(2:end-1,:)';%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                    time_new(ind_f:ind_f+numel(ode_step)-3) = ode_step(2:end-1);
                    ind_f = ind_f +numel(ode_step)-2;
                end
            end
            X_full(:,k+1) = X_aug;
            ...
            if (estimated_coeff.rho_DMC || parameters.empirical)  && (k > 0 && (t_meas(k+1) - t_meas(k)) < stoch_threshold)
...
%             trV(k+1) = tr_V(k+1);
            ...
            % Cd calc
            %             [~,Cd_est(k+1),~] = propagator_dmc(time_pred_utc(k+1),X_full(:,k+1),parameters);
            %             if time_prop_utc(i) >  6000
            %                         [Af_calc,Bf_calc] = gsim_estimation_func(Order_b,Order_o,Xf_name_all,X_ref(:,k+1),P_up,parameters,time_prop(k+1),oxa,Af_total,Bf_total);
            %                         Cd_bias(k+1) = Af_calc(1);
            %                         parameters.Cd_est = Cd_bias(k+1);
            %             end
            %% Smoother storing
    ...
    %% Smoother
    write_to_api(sprintf("' Stage %i: Backward Smoothing'", iteration),flag_disp)
    kk = numel(t_meas);
...
%     write_to_api(sprintf("' Stage %i: Estimated correction is %%0A Position = [%d, %d, %d] m %%0A Velocity = [%d, %d, %d] m/s %%0A SRP Fourier coefficients = [%d, %d, %d] %%0A Density process correction = [%d, %d, %d] '",...
%         iteration, xcorr(1:3)', xcorr(4:6)', xcorr(7:9)', xcorr(10:12)'),flag_disp)
