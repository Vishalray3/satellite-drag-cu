...
function [C, S] = tides(C, S, time_et, jdTT_ref, jd_ut1, TEI, pos_Moon, pos_Sun, r_ref, gm, mu_m,mu_s, x_pole, y_pole, flag_tides, coeff0, coeff1, coeff2)

% convert moon and sun eci positions to ecef
pos_Moon = TEI*pos_Moon;
...
%%%%%%%%% Elastic Earth%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     dCnm20 = (0.29525/5)*( (mu_m/gm)*((r_ref/rM)^3)*lgM(3,1)...
%            + (mu_s/gm)*((r_ref/rS)^3)*lgS(3,1) );
%     dCnm21 = (0.29470/5)*( (mu_m/gm)*((r_ref/rM)^3)*lgM(3,2)*cos(lM)...
%            + (mu_s/gm)*((r_ref/rS)^3)*lgS(3,2)*cos(lS) );
%     dSnm21 = (0.29470/5)*( (mu_m/gm)*((r_ref/rM)^3)*lgM(3,2)*(sin(lM))...
%            + (mu_s/gm)*((r_ref/rS)^3)*lgS(3,2)*(sin(lS)) );
%     dCnm22 = (0.29801/5)*( (mu_m/gm)*((r_ref/rM)^3)*lgM(3,3)*cos(2*lM)...
%            + (mu_s/gm)*((r_ref/rS)^3)*lgS(3,3)*cos(2*lS) );
%     dSnm22 = (0.29801/5)*( (mu_m/gm)*((r_ref/rM)^3)*lgM(3,3)*(sin(2*lM))...
%            + (mu_s/gm)*((r_ref/rS)^3)*lgS(3,3)*(sin(2*lS)) );
%     dCnm30 = (0.093/7)*( (mu_m/gm)*((r_ref/rM)^4)*lgM(4,1)...
%            + (mu_s/gm)*((r_ref/rS)^4)*lgS(4,1) );
%     dCnm31 = (0.093/7)*( (mu_m/gm)*((r_ref/rM)^4)*lgM(4,2)*cos(lM)...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    theta_g = gmst_iers10(jdTT_ref, jd_ut1);
