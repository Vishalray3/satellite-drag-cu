...
% addpath('/Users/vishalray/GoogleDrive/Vishal/PhD/Simulations/Density inversion methods/Spire_Pseudorange_Processing')
...
parameters.drag      = 1;
parameters.tides     = 1;
parameters.flag_tides.SolidEarthTides = 1;
parameters.flag_tides.OceanTides = 1;
parameters.srp       = 1;
...
X_true_aug = interp1(ode_step, X1,sod_sec_sp3 - sod_sec_sp3(1),'spline');
...
for ii = 1:numel(time_prop)
    ...
    [~,Cd_est(ii),rho(ii), theta(ii), phi(ii),a_drag(:,ii),a_srp(:,ii),a_earthrad(:,ii)] = ...
        propagator_truth(time_prop_utc(ii),X1(ii,:)',parameters);
end