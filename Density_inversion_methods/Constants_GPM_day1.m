...
load GPM_GPSdata_201701_06
load GPM_Quatdata_201701_06
load ephemeris_2017_jan1

load nrlmsis_data_2016-17
load GravCoeff_EGM2008_zeroTide

load('Cd_GPMreal_data1_fourier','Af_total_mat','Af_std','Bf_total_mat','Bf_std','Cf_total_mat','Cf_std','Df_total_mat','Df_std')
...
if strcmp(case_run, 'truth')
    ...
    estimated_coeff = [{'rho_DMC'},{'Cd'}]; %[{'rho_DMC'},{'Cd'}];
...
end_index     = 17220;                                  %% last index till which data is being processed
...
tai_time_gps  =  Tgps;  %% offset gets compensated by differencing with GPS epoch since
tai_time_gps  = tai_time_gps + tai_gps_epoch ;
et_time_gps   = cspice_unitim(tai_time_gps', 'TAI', 'TDB');
time_prop     = et_time_gps(1:end_index);
...
%% GPM Parameters data for day 1 (Jan 1 2017)
mass = 3501.6127542096;                           %% in kg before the maneuver of Feb 1
...
epoch_prev = 'Dec 31, 2016 15:59:59.079 UTC';
...
t_req = 0 + delt;                          %% see the ephemeris data to change this
...
zeta =  0.8;%0.2862;%0.2019; % 0.01; %
omega = 0.0018; %2*2*pi/T_orb; %6.5e-4; %0.0018; %2*pi/T_orb; % 2*2*pi/T_orb;
tau_inv = 0; %1/100;
sig_dmc = sqrt(5.5e-22);
q_dmc = 1;
c1 = 1e-7;%1e-17; % 1e-17; %sqrt(4*omega^3*zeta*sig_dmc^2/q_dmc);
c2 = 1e-7; %1e-18;%1e-16; %1e-16;
c3 = 1e-7;%1e-18;
...
GPS_pos = [Xgps(1:end_index)';Ygps(1:end_index)';Zgps(1:end_index)'];                  %% GPS positions in ITRF frame m
GPS_vel = [Vxgps(1:end_index)';Vygps(1:end_index)';Vzgps(1:end_index)'];               %% GPS velocities in ITRF frame m/s
GPS_meas_ecef = [GPS_pos; GPS_vel];
%% Using the quaternion data
q_ecib = [ q4_ecib, q1_ecib, q2_ecib, q3_ecib];            %% first component is scalar
...
for ii = 1:end_index
    ...
    if ismember('CdTrue',estimated_coeff)
        ...
        theta(6:7,ii) = theta(6:7,ii) - SP_neg(ii);            % -y-axis canted panel
        theta(8:9,ii) = theta(8:9,ii) + SP_pos(ii);            % +y-axis feathered panel
    else
        theta(ii) = atan2d(V_b(3),V_b(1)) - SP_neg(ii);
        phi = 0;
    end
    angle_panels(ii,1,:) = zeros(numel(Area_plates),1);       % angle of panels w.r.t the body frame for panel based SRP
    angle_panels(ii,1,6:7) = SP_neg(ii);
    angle_panels(ii,1,8:9) = SP_pos(ii);
end
