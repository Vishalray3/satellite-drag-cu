...
load('Truth_300km_demo', 'theta','rot_SBF2ECI','phi','rho','reci','veci')
...
mass = 98131;                          %% in kg before the maneuver of Aug 9
area = 650;                                         %% change it for order 0
...
%         load(strcat('NRLMSISE_',num2str(year)))
    ...
    case 'HASDM'
        load 2006_HASDM_300-375KM                 %%%%%%%%%%%%% check for different altitudes
        ind_day = [];
        for nn = 1:n_days
            doy_hasdm = doy+nn-1;
            ind_day  = [ind_day;find(den_mat(:,2) == doy_hasdm)];
        end
        ind_alt1 = find(den_mat(:,7) == 375);
        ind_alt2 = find(den_mat(:,7) == 350);
      ...
      load 2006_HASDM_400-475KM
        ind_day = [];
        for nn = 1:n_days
            doy_hasdm = doy+nn-1;
            ind_day  = [ind_day;find(den_mat(:,2) == doy_hasdm)];
        end
        ind_alt1 = find(den_mat(:,7) == 400);
        ind_day_alt1 = intersect(ind_day, ind_alt1);
